...
// Light tooltip style matching white card background
const TOOLTIP_STYLE = {
...
export default function AdminAnalyticsClient() {
  ...
  if (!stats) {
    return <p className="text-[#9e9ea7] text-center py-20">Failed to load analytics.</p>;
  }
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Header */}
      <div>
        <h1 className="text-2xl font-bold text-[#0d0d0d] tracking-tight">Analytics</h1>
        <p className="text-[#6b6b76] text-sm mt-1">Platform performance and revenue insights</p>
      </div>
      ...
      <div className="grid grid-cols-2 xl:grid-cols-4 gap-4">
        {[
          ...
          <div key={label} className="drib-card p-4 hover:shadow-card-hover transition-shadow">
            ...
            <p className="text-[#0d0d0d] font-bold text-2xl">{value}</p>
          </div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
        ...
        <div className="drib-card p-5">
          <h2 className="text-[#0d0d0d] font-semibold mb-4">User Registrations (Last 7 Days)</h2>
          {userGrowthData.length > 0 ? (
            <ResponsiveContainer width="100%" height={220}>
              <LineChart data={userGrowthData}>
                <CartesianGrid strokeDasharray="3 3" stroke="#f0f0f0" />
                <XAxis dataKey="date" tick={{ fill: "#9e9ea7", fontSize: 11 }} axisLine={false} tickLine={false} />
                <YAxis tick={{ fill: "#9e9ea7", fontSize: 11 }} axisLine={false} tickLine={false} allowDecimals={false} />
            ...
            <div className="flex items-center justify-center h-[220px] text-[#9e9ea7] text-sm">
        ...
        {/* Revenue by Method */}
        <div className="drib-card p-5">
          <h2 className="text-[#0d0d0d] font-semibold mb-4">Revenue by Payment Method</h2>
          {revenueMethodData.length > 0 ? (
            <ResponsiveContainer width="100%" height={220}>
              <BarChart data={revenueMethodData} barSize={36}>
                <CartesianGrid strokeDasharray="3 3" stroke="#f0f0f0" />
                <XAxis dataKey="name" tick={{ fill: "#9e9ea7", fontSize: 11 }} axisLine={false} tickLine={false} />
                <YAxis tick={{ fill: "#9e9ea7", fontSize: 11 }} axisLine={false} tickLine={false} />
                <Tooltip contentStyle={TOOLTIP_STYLE}
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
        {/* Plan Distribution */}
        <div className="drib-card p-5">
          <h2 className="text-[#0d0d0d] font-semibold mb-4">User Plan Distribution</h2>
            ...
            <div className="space-y-3">
              {planData.map((d) => (
                <div key={d.name} className="flex items-center gap-2">
                  ...
                  <div>
                    <p className="text-[#0d0d0d] text-sm font-semibold">{d.name}</p>
                    <p className="text-[#9e9ea7] text-xs">{d.value} users</p>
                  </div>
                </div>
              ))}
            </div>
        ...
        {/* Transaction Status */}
        <div className="drib-card p-5">
          <h2 className="text-[#0d0d0d] font-semibold mb-4">Transaction Status</h2>
          <div className="flex items-center gap-6">
            <ResponsiveContainer width="50%" height={200}>
              <PieChart>
            ...
            <div className="space-y-3">
              {txStatusData.map((d) => (
                <div key={d.name} className="flex items-center gap-2">
                  ...
                  <div>
                    <p className="text-[#0d0d0d] text-sm font-semibold">{d.name}</p>
                    <p className="text-[#9e9ea7] text-xs">{d.value} transactions</p>
                  </div>
                </div>
              ))}
            </div>
      ...
      <div className="drib-card p-6">
        <h2 className="text-[#0d0d0d] font-semibold mb-5">Revenue Summary</h2>
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 text-center">
          <div>
            <p className="text-[#6b6b76] text-sm mb-1">Total Revenue</p>
            <p className="text-[#0d0d0d] font-bold text-3xl">K{stats.revenue.total.toLocaleString()}</p>
          </div>
          <div>
            <p className="text-[#6b6b76] text-sm mb-1">This Month</p>
            <p className="font-bold text-3xl" style={{ color: "#ea4c89" }}>K{stats.revenue.thisMonth.toLocaleString()}</p>
          </div>
          <div>
            <p className="text-[#6b6b76] text-sm mb-1">Last Month</p>
            <p className="text-[#0d0d0d] font-bold text-3xl">K{stats.revenue.lastMonth.toLocaleString()}</p>
